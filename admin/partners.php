    ...
    <div class="row" style="flex-wrap: nowrap !important">
      <div class="col-2 dash-nav  bg-info py-2" style="overflow-x:hidden !important; font-size: 14px !important">
                <ul class="list-group dash-nav-ui  bg-info text-white">
                    ...
                    <li class="my-2 list-group-item bg-info text-white rounded active" style="border: none !important;" ><a href="partners.php" class="list-group-item list-group-item-action bg-info text-white px-0" style="border:none !important" ><i style="margin-left:0 !important" class="mx-1 fa-solid fa-key"></i><span>Partners</span></a></li>
                    ...
                    <tbody>
                        ...
                        <?php while($row = mysqli_fetch_assoc($rslt)){ ?>
                            ...
                            <tr>
                                ...
                                <td><?php
                                $sqlpartnerhotel = 'select * from partnerhotel where partner_id = '.$row['id'];
                                $rsltpartnerhotel = mysqli_query($conn, $sqlpartnerhotel);
                                $rowpartnerhotel = mysqli_fetch_assoc($rsltpartnerhotel);
                                if(!$rowpartnerhotel){
                                    echo 'No hotel';
                                }else{
                                $sqlhotel = 'select * from hotel where id = '.$rowpartnerhotel['hotel_id'];
                                $rslthotel = mysqli_query($conn, $sqlhotel);
                                $rowhotel = mysqli_fetch_assoc($rslthotel);
                                $sqlcountries = 'select * from countries where countries_id = '.$rowhotel['countries_id'];
                                $rsltcountries = mysqli_query($conn, $sqlcountries);
                                $rowcountries = mysqli_fetch_assoc($rsltcountries);
                                $checkin = date('Y-m-d', strtotime('now'));
                                $checkout = date('Y-m-d', strtotime('+1 day'));
                                $adults = 1;
                                $children = 0;
                                $rooms = 1;
                                ?>
                                <a class="text-white" href="../hotel.php?hotel_id=<?php echo $rowhotel['id'] ?>&destination=<?php echo $rowcountries['name'] ?>&checkin=<?php echo $checkin ?>&checkout=<?php echo $checkout ?>&adults=<?php echo $adults ?>&children=<?php echo $children ?>&rooms=<?php echo $rooms ?>"><?php echo $rowhotel['name']; ?></a>
                                <?php
                                }
...
if(isset($_POST['delete'])){
    ...
    echo "<script>location.href = 'partners.php'</script>";
}
?>
